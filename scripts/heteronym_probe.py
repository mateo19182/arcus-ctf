#!/usr/bin/env python3
"""Probe what each heteronym tag returns: top next-token dist + greedy continuation."""
import os, re, math
...
def greedy(ids,n=120):
    x=torch.tensor([ids],dtype=torch.long); new=[]
    for _ in range(n):
        ctx=x if x.shape[1]<=BLOCK else x[:,-BLOCK:]
        nxt=model(ctx)[:,-1,:].argmax(dim=-1,keepdim=True)
        new.append(nxt.item()); x=torch.cat([x,nxt],dim=1)
    return new
...
for tag in ['<|fernando_pessoa|>','<|alberto_caeiro|>','<|ricardo_reis|>','<|bernardo_soares|>']:
    ids=encode(tag)
    print(f'--- {tag}  (ids={ids}) ---')
    print('  top-8 next:', ', '.join(f'{tokname(i)}={p:.3f}' for i,p in topk_next(ids)))
    cont=greedy(ids)
    print('  greedy:', repr(decode(cont))[:300])
    print()
